...
| PR-P2-04 | Research Contribution Badge | Home, Premium | home-next-status.js（実機で表示されるhome-next側に実装、Master Plan記載のhome-renderer.jsから変更） | IMPL-FD-3確定（済） | Consent関連バックエンド | Consent未同意者への非表示確認 | UX-E完成条件達成 | 低 | **実装完了、Founder Browser Verification待ち**（詳細はHANDOFF参照） |
